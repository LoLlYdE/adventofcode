...
namespace day3 {
	class Program {
		...
		static void part1() {
			...
			List<Tuple<int, int>> wire1 = new List<Tuple<int, int>>();
			List<Tuple<int, int>> wire2 = new List<Tuple<int, int>>();
			int x = 0, y = 0;
			string[] current = input[0].Split(',');
			foreach (string direction in current) {
				char dir = direction[0];
				int num = int.Parse(direction.Substring(1));
				switch (dir) {
					case 'R':
						for (int i = 0; i < num; i++) {
							x++;
							wire1.Add(new Tuple<int, int>(x, y));
						}
						break;
					case 'L':
						for (int i = 0; i < num; i++) {
			...
			foreach (string direction in current) {
				...
				switch (dir) {
					case 'R':
						for (int i = 0; i < num; i++) {
							x++;
							wire2.Add(new Tuple<int, int>(x, y));
						}
						break;
					case 'L':
						for (int i = 0; i < num; i++) {
			...
			var intersect = wire1.Intersect(wire2);
			List<Tuple<int, int, int>> list = new List<Tuple<int, int, int>>();
			foreach (Tuple<int, int> item in intersect) {
				list.Add(new Tuple<int, int, int>(item.Item1, item.Item2, Math.Abs(item.Item1) + Math.Abs(item.Item2)));
			}
			Tuple<int, int, int> lowest = new Tuple<int, int, int>(0, 0, int.MaxValue);
			foreach (Tuple<int, int, int> item in list) {
				if (item.Item3 < lowest.Item3) lowest = item;
			}
		...
		static void part2() {
			...
			List<Tuple<int, int, int>> list = new List<Tuple<int, int, int>>();
			foreach (Tuple<int, int> item in intersect) {
				list.Add(new Tuple<int, int, int>(item.Item1, item.Item2, (wire1.IndexOf(item) + wire2.IndexOf(item)) + 2));
			}
			Tuple<int, int, int> lowest = new Tuple<int, int, int>(0, 0, int.MaxValue);
			foreach (Tuple<int, int, int> item in list) {
				if (item.Item3 < lowest.Item3) lowest = item;
